...
class YIFYSubtitles {
	...
	func getSubtitle(_ imdb_id : String, onCompletion: @escaping ([String : String]) -> Void) {
		let parameters = ["type" : "movie"]
		RestApiManager.sharedInstance.getJSONFromURL(YIFYSubtitles.apiEndpoint + imdb_id, parameters: parameters as [String : AnyObject]) { (json) in
			...
			if let _ = json["success"].bool {
				...
				for (lang, subs) in json["subs"][imdb_id] {
					...
					for (_, knownLangFull) in ButterAPIManager.languages {
						if lang == knownLangFull.lowercased() {
							var bestSub : JSON?
